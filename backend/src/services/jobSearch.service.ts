...
import { getUserProfile, getPrimaryRoadmap } from "../data-access/user.repository"; // Assume getPrimaryRoadmap exists
import { searchJobsAPI } from "../tools/serpapi";
import { JobSearchOutputSchema } from "../utils/zodSchemas";
...
export async function findPersonalizedJobs(userId: string, query: string) {
  const userProfile = await getUserProfile(userId);
  const roadmap = await getPrimaryRoadmap(userId); // You may need to implement this repository function
  ...
  const rawJobListings = await searchJobsAPI(generatedQuery);
  ...
  function fixJobDetails(arr: any[]) {
    return arr.map(job => ({
      ...job,
  ...
  if (finalResult.passionRoles) finalResult.passionRoles = fixJobDetails(finalResult.passionRoles);
  if (finalResult.strengthRoles) finalResult.strengthRoles = fixJobDetails(finalResult.strengthRoles);
  if (finalResult.growthRoles) finalResult.growthRoles = fixJobDetails(finalResult.growthRoles);

  return JobSearchOutputSchema.parse(finalResult);
}
